...
export async function GET(request: Request) {
  ...
  try {
    ...
    return new Response(response.body, { headers: { "Content-Type": response.headers.get("content-type") ?? "image/png", "Cache-Control": "public, s-maxage=300, stale-while-revalidate=900" } });
  } catch {
