...
export async function GET() {
  ...
  const assessment = assessThreat({ windGustMph, hourlyRainIn, radarDbz, pressureTrendInHgPerHr, alertsHere: alerts.here, alertsNearby: alerts.nearby });
  return Response.json({ assessment, inputs: { windGustMph, hourlyRainIn, radarDbz, pressureTrendInHgPerHr, stationOnline: snapshot !== null, radarFrameTime: latestFrame?.observedAt ?? null, alertCountHere: alerts.here.length, alertCountNearby: alerts.nearby.length } }, { headers: { "Cache-Control": "public, s-maxage=60, stale-while-revalidate=120" } });
}
