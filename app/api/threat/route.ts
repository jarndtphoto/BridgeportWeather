...
export async function GET() {
  const [ambientResult, framesResult, alertsResult, probSevereResult] = await Promise.allSettled([
    getAmbientSnapshot(),
    getRadarFrames(),
    getNwsAlerts(BRIDGEPORT_LAT, BRIDGEPORT_LON, STATE_CODE),
    getProbSevereNear(BRIDGEPORT_LAT, BRIDGEPORT_LON),
  ]);
  ...
  const assessment = assessThreat({
    ...
    probSevereStorm: probSevere.storm,
  });
  return Response.json({
    ...
    inputs: {
      ...
      stormTrend: evolution.trend,
      pressureTrendInHgPerHr,
      stationOnline: snapshot !== null,
      radarFrameTime: latestFrame?.observedAt ?? null,
      alertCountHere: alerts.here.length,
      alertCountNearby: alerts.nearby.length,
    },
