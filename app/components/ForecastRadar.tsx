...
export default function ForecastRadar(){
 ...
 const [frameIndex,setFrameIndex]=useState(0),[playing,setPlaying]=useState(false),[forecastLayer,setForecastLayer]=useState<ForecastLayer>("precipitation"),[mapReady,setMapReady]=useState(false),[modelInitUtc,setModelInitUtc]=useState<string|null>(null),[checkedAt,setCheckedAt]=useState<string|null>(null),[touchMap,setTouchMap]=useState(false),[mapInteraction,setMapInteraction]=useState(false),[viewRevision,setViewRevision]=useState(0),[tabActive,setTabActive]=useState(false);
 ...
 useEffect(()=>{
  ...
  const sync=()=>{
   ...
   setTabActive(active);
   if(active){setPlaying(false);setFrameIndex(0);requestAnimationFrame(()=>{mapRef.current?.invalidateSize(false);setViewRevision(v=>v+1)})}
   else setPlaying(false);
  };
  sync();
  input.addEventListener("change",sync);
  document.addEventListener("visibilitychange",sync);
  return()=>{input.removeEventListener("change",sync);document.removeEventListener("visibilitychange",sync)};
 },[]);
 ...
 useEffect(()=>{const map=mapRef.current,L=leafletRef.current;if(!map||!L||!mapReady||!modelInitUtc||!tabActive)return;const token=++renderTokenRef.current,frameMinutes=forecastMinutes[frameIndex];
  const promote=(next:Layer,targetOpacity:number)=>{if(token!==renderTokenRef.current){if(map.hasLayer(next))map.removeLayer(next);return}const old=modelRef.current;modelRef.current=next;fadeLayer(next,targetOpacity);window.setTimeout(()=>{if(old&&old!==next&&map.hasLayer(old))map.removeLayer(old)},FADE_MS+60)};
  if(forecastLayer==="precipitation"){const frame=String(frameMinutes).padStart(4,"0"),run=iemRunId(modelInitUtc),tile:any=L.tileLayer(`https://mesonet.agron.iastate.edu/cache/tile.py/1.0.0/hrrr::REFP-F${frame}-${run}/{z}/{x}/{y}.png`,{pane:"weather",opacity:0,maxZoom:12,attribution:"HRRR via Iowa Environmental Mesonet"}).addTo(map);let promoted=false;const ready=()=>{if(promoted)return;promoted=true;promote(tile,.82)};tile.once("load",ready);const fallback=window.setTimeout(ready,1400);return()=>{window.clearTimeout(fallback);tile.off("load",ready);if(token!==renderTokenRef.current&&map.hasLayer(tile))map.removeLayer(tile)}}
  const bounds=map.getBounds(),sw=L.CRS.EPSG3857.project(bounds.getSouthWest()),ne=L.CRS.EPSG3857.project(bounds.getNorthEast()),size=map.getSize();if(size.x<100||size.y<100)return;const params=new URLSearchParams({bbox:`${sw.x},${sw.y},${ne.x},${ne.y}`,width:String(size.x),height:String(size.y),modelInitUtc,forecastMinutes:String(frameMinutes)});const image:any=L.imageOverlay(`/api/forecast-cloud/image?${params}`,bounds,{opacity:0,pane:"weather",interactive:false}).addTo(map);let promoted=false;const ready=()=>{if(promoted)return;promoted=true;promote(image,.9)};image.once("load",ready);const fallback=window.setTimeout(ready,1400);return()=>{window.clearTimeout(fallback);image.off("load",ready);if(token!==renderTokenRef.current&&map.hasLayer(image))map.removeLayer(image)}
 },[forecastMinutes,frameIndex,forecastLayer,mapReady,modelInitUtc,viewRevision,tabActive]);
